...
OUT = ROOT / "demo"
TARGET = OUT / "policypilot_demo_2026-09-07.webm"
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        page.screenshot(path=str(OUT / "policypilot_start.png"), full_page=False)

        page.select_option("#scenario", "no_gaming")
        page.get_by_role("button", name="Run 14-day policy test").click()
        page.wait_for_timeout(1300)
        assert page.locator("#scenarioTag").get_by_text("Gaming removed").is_visible()
        page.screenshot(path=str(OUT / "policypilot_gaming_removed.png"), full_page=False)

        page.get_by_role("button", name="Action learner").click()
        page.wait_for_timeout(1300)
        assert page.get_by_text("Same policy, another learner").is_visible()
        page.screenshot(path=str(OUT / "policypilot_counterfactual.png"), full_page=False)

        with page.expect_download() as download_info:
            page.get_by_role("button", name="Export scenario").click()
        download = download_info.value
        assert download.suggested_filename == "policypilot-scenario.json"
        download.save_as(str(OUT / "policypilot-scenario.json"))
